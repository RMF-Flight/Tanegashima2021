...
if __name__ == "__main__":
    ...
    gps = MicropyGPS(9, 'dd') # 引数はタイムゾーンの時差と出力フォーマット
    ser = serial.Serial('/dev/serial0', 9600, timeout=10)   #9600

    def rungps(): # GPSモジュールを読み、GPSオブジェクトを更新する
        ser.readline() # 最初の1行は中途半端なデーターが読めることがあるので、捨てる
        while True:
            a = ser.readline()
            if sys.getsizeof(a) < 100:      #センテンスがくっつくことがあるので回避のため追加
                try:
                    sentence = a.decode('utf-8') # GPSデーターを読み、文字列に変換する
                except:
                    continue
